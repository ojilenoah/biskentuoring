...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section
        id="home"
        className="relative overflow-hidden bg-gradient-to-br from-primary/5 via-background to-primary/10 py-12 px-4"
      >
        <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-transparent"></div>
        <div className="container mx-auto max-w-5xl relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 items-stretch">
            <div className={`space-y-6 relative z-10 ${isVisible ? "animate-slide-in-left" : "opacity-0"}`}>
            ...
            <div className={`relative z-0 ${isVisible ? "animate-slide-in-right" : "opacity-0"}`}>
              <div className="relative group cursor-pointer overflow-hidden h-full" onClick={handleHeroCardClick}>
                <div className="relative rounded-2xl overflow-hidden shadow-2xl h-full">
                  {heroItems.map((item, index) => (
                    <div
                      key={index}
                      className={`absolute inset-0 w-full h-full transition-opacity duration-700 ease-in-out ${
                        index === heroCardIndex ? "opacity-100" : "opacity-0"
                      }`}
                    >
                      {item.type === 'image' ? (
                        <img
                          ...
                          loading={index === heroCardIndex ? "eager" : "lazy"}
                          decoding="async"
          ...
          <div className="-mx-4 px-4 overflow-x-auto scrollbar-hide pb-4">
            <div className="flex gap-3 min-w-max">
              {tutors.map((tutor, index) => (
                <Card
                  key={index}
                  className="p-0 flex-shrink-0 w-64 hover:shadow-lg transition-all duration-300 hover:-translate-y-2 scroll-animate-scale overflow-hidden rounded-lg"
                ...
                >
                  ...
                  <CardContent className="p-2 text-left">
                    <h3 style={{ marginBottom: 10 }} className="font-semibold text-base">{tutor.name}</h3>
                    <div className="flex flex-wrap gap-2 mb-2">
                      {(tutor.tags || []).map((tag: string, ti: number) => (
                        <button
                          key={ti}
                          onClick={() => setSelectedTutor(tutor)}
                          className="text-xs text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded-full"
                        >
                          {tag}
                        </button>
                      ))}
                    </div>
                    <p className="text-sm text-muted-foreground leading-snug max-h-24 overflow-hidden">{tutor.description}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
